...
def train_model(method):
    ...
    if method == "silme":
        ...
        df = df.select_dtypes(include=[np.number]).interpolate(method='linear')
    else:
    ...
    with open(f'C:\\Users\\User\\Desktop\\ML_Proje\\b\\{method}_logistic_regression_metrics.txt', 'w', encoding='utf-8') as f:
        f.write(metrics)
    ...
    plt.ylabel('Gerçek')
    plt.title(f'Karışıklık Matrisi ({method})')
    matrix_path = f'C:\\Users\\User\\Desktop\\ML_Proje\\b\\{method}_logistic_regression_cm.png'
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        print("Lütfen bir eksik doldurma yöntemi belirtin: silme, ffill, bfill, sifirla, mean")
