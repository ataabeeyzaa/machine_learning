...
resampled_data.to_csv('C:/Users/User/Desktop/ML_Proje/b/ros_resampled_data.csv', index=False)
...
joblib.dump(model_resampled, 'C:/Users/User/Desktop/ML_Proje/b/ros_random_forest_model.pkl')
joblib.dump(scaler, 'C:/Users/User/Desktop/ML_Proje/b/ros_scaler.pkl')
joblib.dump(X.columns.tolist(), 'C:/Users/User/Desktop/ML_Proje/b/feature_names.pkl')

# Metrikleri kaydet
output_dir = 'C:/Users/User/Desktop/ML_Proje/b'
with open(os.path.join(output_dir, 'ros_metrics_unbalanced_random_forest.txt'), 'w') as f:
    for metric, value in metrics_unbalanced.items():
        f.write(f'{metric}: {value}\n')
with open(os.path.join(output_dir, 'ros_metrics_balanced_random_forest.txt'), 'w') as f:
...
plt.title('Dengesiz Veri Seti - Karışıklık Matrisi (ROS - Random Forest)')
plt.xlabel('Tahmin Edilen')
plt.ylabel('Gerçek')
plt.savefig(os.path.join(output_dir, 'ros_conf_matrix_unbalanced_random_forest.png'))
plt.close()
...
plt.title('Dengesiz Veri Seti - Performans Metrikleri (ROS - Random Forest)')
plt.xlabel('Metrik')
plt.ylabel('Değer')
plt.ylim(0, 1)
plt.savefig(os.path.join(output_dir, 'ros_metrics_unbalanced_random_forest.png'))
plt.close()

# Karışıklık Matrisi Görselleştir (Dengeli)
sns.heatmap(conf_matrix_balanced, annot=True, fmt='d')
plt.title('Dengeli Veri Seti - Karışıklık Matrisi (ROS - Random Forest)')
plt.xlabel('Tahmin Edilen')
plt.ylabel('Gerçek')
plt.savefig(os.path.join(output_dir, 'ros_conf_matrix_balanced_random_forest.png'))
plt.close()

# Performans Metrikleri Görselleştir (Dengeli)
plt.figure(figsize=(10, 5))
sns.barplot(x=list(metrics_balanced.keys()), y=list(metrics_balanced.values()))
plt.title('Dengeli Veri Seti - Performans Metrikleri (ROS - Random Forest)')
plt.xlabel('Metrik')
plt.ylabel('Değer')
plt.ylim(0, 1)
plt.savefig(os.path.join(output_dir, 'ros_metrics_balanced_random_forest.png'))
plt.close()
