...
with open(os.path.join(output_dir, 'ros_kfold_knn_fold_metrics.txt'), 'w', encoding='utf-8') as f:
    ...
    for fold, (train_index, test_index) in enumerate(kfold_splits):
        ...
        plt.ylabel('Gerçek')
        plt.title(f'KNN K-Fold Karışıklık Matrisi - Fold {fold + 1} (ROS)')
        plt.savefig(os.path.join(output_dir, f'ros_kfold_knn_conf_matrix_fold_{fold + 1}.png'))
        plt.close()
...
with open(os.path.join(output_dir, 'ros_kfold_knn_metrics.txt'), 'w', encoding='utf-8') as f:
    f.write(metrics)
