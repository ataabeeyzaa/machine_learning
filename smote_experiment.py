...
with open(os.path.join(output_dir, 'smote_metrics_unbalanced.txt'), 'w') as f:
    for metric, value in metrics_unbalanced.items():
        f.write(f'{metric}: {value}\n')
with open(os.path.join(output_dir, 'smote_metrics_balanced.txt'), 'w') as f:
...
plt.title('Dengesiz Veri Seti - Karışıklık Matrisi (SMOTE)')
plt.xlabel('Tahmin Edilen')
plt.ylabel('Gerçek')
plt.savefig(os.path.join(output_dir, 'smote_conf_matrix_unbalanced.png'))
plt.close()
...
plt.title('Dengesiz Veri Seti - Performans Metrikleri (SMOTE)')
plt.xlabel('Metrik')
plt.ylabel('Değer')
plt.ylim(0, 1)
plt.savefig(os.path.join(output_dir, 'smote_metrics_unbalanced.png'))
plt.close()

# Karışıklık Matrisi Görselleştir (Dengeli)
sns.heatmap(conf_matrix_balanced, annot=True, fmt='d')
plt.title('Dengeli Veri Seti - Karışıklık Matrisi (SMOTE)')
plt.xlabel('Tahmin Edilen')
plt.ylabel('Gerçek')
plt.savefig(os.path.join(output_dir, 'smote_conf_matrix_balanced.png'))
plt.close()

# Performans Metrikleri Görselleştir (Dengeli)
plt.figure(figsize=(10, 5))
sns.barplot(x=list(metrics_balanced.keys()), y=list(metrics_balanced.values()))
plt.title('Dengeli Veri Seti - Performans Metrikleri (SMOTE)')
plt.xlabel('Metrik')
plt.ylabel('Değer')
plt.ylim(0, 1)
plt.savefig(os.path.join(output_dir, 'smote_metrics_balanced.png'))
plt.close()
