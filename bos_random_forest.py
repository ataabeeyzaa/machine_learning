...
X_resampled, y_resampled = bos.fit_resample(X, y)

# ROS ile yeniden örneklenen veri setini kaydet
resampled_data = pd.DataFrame(X_resampled, columns=X.columns)
resampled_data['Survived'] = y_resampled
resampled_data.to_csv('C:/Users/User/Desktop/ML_Proje/b/bos_resampled_data.csv', index=False)
...
with open(os.path.join(output_dir, 'bos_metrics_unbalanced_random_forest.txt'), 'w') as f:
    for metric, value in metrics_unbalanced.items():
        f.write(f'{metric}: {value}\n')
with open(os.path.join(output_dir, 'bos_metrics_balanced_random_forest.txt'), 'w') as f:
...
plt.title('Dengesiz Veri Seti - Karışıklık Matrisi (BOS - Random Forest)')
plt.xlabel('Tahmin Edilen')
plt.ylabel('Gerçek')
plt.savefig(os.path.join(output_dir, 'bos_conf_matrix_unbalanced_random_forest.png'))
plt.close()
...
plt.title('Dengesiz Veri Seti - Performans Metrikleri (BOS - Random Forest)')
plt.xlabel('Metrik')
plt.ylabel('Değer')
plt.ylim(0, 1)
plt.savefig(os.path.join(output_dir, 'bos_metrics_unbalanced_random_forest.png'))
plt.close()

# Karışıklık Matrisi Görselleştir (Dengeli)
sns.heatmap(conf_matrix_balanced, annot=True, fmt='d')
plt.title('Dengeli Veri Seti - Karışıklık Matrisi (BOS - Random Forest)')
plt.xlabel('Tahmin Edilen')
plt.ylabel('Gerçek')
plt.savefig(os.path.join(output_dir, 'bos_conf_matrix_balanced_random_forest.png'))
plt.close()

# Performans Metrikleri Görselleştir (Dengeli)
plt.figure(figsize=(10, 5))
sns.barplot(x=list(metrics_balanced.keys()), y=list(metrics_balanced.values()))
plt.title('Dengeli Veri Seti - Performans Metrikleri (BOS - Random Forest)')
plt.xlabel('Metrik')
plt.ylabel('Değer')
plt.ylim(0, 1)
plt.savefig(os.path.join(output_dir, 'bos_metrics_balanced_random_forest.png'))
plt.close()
