...
feature_names = joblib.load('C:/Users/User/Desktop/ML_Proje/b/zscore_feature_names.pkl')
model = joblib.load('C:/Users/User/Desktop/ML_Proje/b/zscore_random_forest_model.pkl')
scaler = joblib.load('C:/Users/User/Desktop/ML_Proje/b/zscore_scaler.pkl')

# Giriş verilerinin doğru şekilde işlenmesi
user_input = pd.DataFrame([[int(sinif),
                            0 if cinsiyet == 'female' else 1,  # Sex column: 0 if female, 1 if male
                            float(yas),
                            int(kardes),
                            int(ebeveyn),
                            float(bilet),
...
# Sadece 'Age' ve 'Fare' kolonlarında ölçeklendirme yapalım
